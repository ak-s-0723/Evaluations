...
public class ProductSearchService {
    ...
    public Slice<Product> findProductsWhichCanBeSkippedInLuggageBecauseOfWeightLimit(Double allowedLimit, Double actualWeight,Integer pageNumber,Integer pageSize) {
        Double difference = actualWeight - allowedLimit;
        Sort sort = Sort.by("amount").descending().and(Sort.by("weight"));
        return productRepository.findProductByWeightLessThanEqual(difference, PageRequest.of(pageNumber,pageSize,sort));
    }


    public Slice<Product> findProductsBelongingToCategoryDisplayedByAge(String category,Integer pageNumber,Integer pageSize) {
