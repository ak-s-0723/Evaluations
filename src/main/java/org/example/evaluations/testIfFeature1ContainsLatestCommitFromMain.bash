...
COMMIT_MESSAGE="Added Log line in main method"
SOURCE_BRANCH="main"
...
if git log "$TARGET_BRANCH" --grep="$COMMIT_MESSAGE" --format="%H" | grep -q "$COMMIT_HASH"; then
    echo "Commit with message '$COMMIT_MESSAGE' exists in branch '$TARGET_BRANCH'."
else
    echo "Error : Commit with message '$COMMIT_MESSAGE' is MISSING in branch '$TARGET_BRANCH'."
    exit 1
fi
